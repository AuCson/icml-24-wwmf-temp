...
from memory_utils.multi_dataset import MultiTaskMemory, DatasetMemory, CoresetMemory
import torch
...
class MyTrainer(Trainer):
    def __init__(self, **kwargs):
        ...
        self.memory: MultiTaskMemory = kwargs.pop('memory')
        self.past_model_creator = kwargs.pop('past_model_creator', None)
        if self.config.cl_use_distill or self.config.use_reg_loss:
    ...
    def model_update_over_stream(self, model, ocl_batches, optimizer, n_step, eval_mode=False):
        ...
        for batch in ocl_batches:
            batch_ = self.clean_batch(batch)
            if eval_mode:
                loss_dt = self.training_step_eval_mode(model, batch_, use_reg_loss=self.config.use_reg_loss)
            else:
                loss_dt = self.training_step(model, batch_)
    ...
    def nstep_model_update(self, model, batch, optimizer, n_step, do_replay=False, do_retrieve=False, pt_ds=None,
        ...
        elif config.delay_opt:
            replay_logger.info('Cannot use separate optimizers when opt steps are delayed')
            replay_optimizer = optimizer
        ...
        if do_replay:
            if self.config.use_coreset:
                memory = CoresetMemory(pt_ds, config.pt_tasks, self.data_collator, self.coreset_info)
            else:
    ...
    def training_step_eval_mode(self, model, inputs, **kwargs) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs, kwargs.get('use_reg_loss', False))
    ...
    def compute_loss(self, model, inputs, use_reg_loss=False, return_outputs=False, keep_logit_topk=-1):
        """
        ...
        if use_reg_loss:
            with torch.no_grad():
                prev_logits= self.get_before_logits(self.past_model, inputs, eval_mode=True)
            reg_loss = self.config.reg_loss_alpha * self.logit_dist_loss(outputs.logits, prev_logits.detach(), inputs['labels'])
            replay_logger.info('Reg loss: {}'.format(reg_loss.item()))
            loss += reg_loss

        return (loss, outputs) if return_outputs else loss

    def logit_dist_loss(self, before_logits, after_logits, labels):
        # [B,T,V], [B,T,V], [B,T]
        labels_noneg = labels.clone()
        labels_isneg = labels == -100 # [B,T]
        labels_noneg[labels == -100] = 0
        loss_mask = torch.ones_like(before_logits).long() # [B,T,V]
        for b in range(loss_mask.size(0)):
            # [T,V], [T]
            loss_mask[b, torch.arange(loss_mask.size(1)), labels_noneg[b]] = 0
            loss_mask[b, labels_isneg[b]] = 0

        l2_dist = (after_logits - before_logits) ** 2
        l2_dist_masked = l2_dist * loss_mask.float()

        denom = loss_mask.sum()
        if denom == 0:
            denom = 1
        l2_dist_masked_norm = l2_dist_masked.sum() / denom

        return l2_dist_masked_norm
