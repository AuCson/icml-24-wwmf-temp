...
def run_evaluate(config, model, trainer: MyTrainer, eval_ds_or_batches):
...
def load_all_records(exp='vanilla_bg100_lr1e-6', task='super_glue-copa', model_type='bart0'):
    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/aff_log.pkl', 'rb') as f:
        aff_obj = pickle.load(f)
        aff_score = np.mean([v for v in get_cnts(aff_obj).values()])

    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/aff_score_log.pkl', 'rb') as f:
        aff_score_obj = pickle.load(f)
        # aff_score = np.mean([v for v in get_cnts(aff_obj).values()])

    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/ocl_log.pkl', 'rb') as f:
        ocl_obj = pickle.load(f)
        succ_score = edit_success_rate(ocl_obj)

    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/ocl_error_ds.csv', 'r') as f:
        reader = csv.reader(f)
        ocl_rows = [_ for _ in reader]

    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/concat_pt_ds.csv', 'r') as f:
        reader = csv.reader(f)
...
def load_loss_records(exp='vanilla_bg100_lr1e-6', task='super_glue-copa', model_type='bart0'):
    with open(f'/home/xsjin/cl-analysis/runs/instance-p3-{model_type}/{exp}/{task}/aff_loss_log.pkl', 'rb') as f:
