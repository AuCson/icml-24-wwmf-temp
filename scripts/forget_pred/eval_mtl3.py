        ...
        for cmb in cmbs:
            ...
            for eval_task in eval_pool:
                cmd = 'python train_logit_predict.py --config_files configs/p3/p3_default.yaml configs/p3/instance-bart0-base-ocl/vanilla_bg100_large_eval_mode.yaml ' \
                      ...
                      '--templates postfix=_fpd_paired_mean_mlp_mtl/step20k/{mtl_tasks} task={eval_task} --ocl_task {eval_task} --do_eval'.format(mtl_tasks=tasks, eval_task=eval_task, LR='1e-6')
                print(cmb)
                print(cmd)
                os.system(cmd)
