from .nli import NLIDataManager
from .nli import Subset
from .posneg import NLIPosNegDataManager

from .p3 import P3Dataset
from .lm import SFTDataset, SFTExampleOnlyDataset

def get_dm_cls(task_type):
    if task_type == 'nli':
        return NLIDataManager
    elif task_type == 'nli_posneg':
        return NLIPosNegDataManager
    else:
...
def load_ocl_ds_by_task_id(config, tokenizer, task_cat, task_id):
    if task_cat == 'mmlu':
        ...
        test_ds = SFTDataset.from_truthful_qa([task],'validation',config,tokenizer) # same, dummy
    elif task_cat == 'tulu_train':
        all_tasks = config.tulu_tasks
        task = all_tasks[task_id]
        train_ds = SFTDataset.from_tulu_train([task], 'train', config, tokenizer)
        test_ds = SFTDataset.from_tulu_train([task], 'train', config, tokenizer)  # same, dummy
    else:
